...
def save_fund(data_array):
    ...
    insertSQL = " insert into etf_rank(fund_id,fund_name,fund_name_abbr,cal_date,net_asset_value,accumulative,oneweek,onemonth,threemonth,sixmonth,oneyear,twoyear,threeyear,thisyear,setup,category,setup_date) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
    value_list = []
    for datas in data_array:
        ...
        oneweek = clean_data(data[6])
        onemonth = clean_data(data[7])
        threemonth = clean_data(data[8])
        sixmonth = clean_data(data[9])
        oneyear = clean_data(data[10])
        twoyear = clean_data(data[11])
        threeyear = clean_data(data[12])
        thisyear = clean_data(data[13])
        setup = clean_data(data[14])
        setup_date= clean_data(data[15])
        value_list.append((fund_id,fund_name,fund_name_abbr,cal_date,net_asset_value,accumulative,oneweek,onemonth,threemonth,sixmonth,oneyear,twoyear,threeyear,thisyear,setup,'ETF-场内',setup_date))
    try:
        #print(value_list)
...
# 开始组合函数
etf_rank_url = constant.ETF_RANK_URL
# 清空表
utils.truncate_table("etf_rank")
# 插入抓取的数据
fund_array = get_fund_array(etf_rank_url)
save_fund(fund_array)
